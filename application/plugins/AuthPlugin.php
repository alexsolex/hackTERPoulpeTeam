...
class Application_Plugin_AuthPlugin extends Zend_Controller_Plugin_Abstract {
    ...
    /**
     * l'id de l'organisme en cours (éventuel)
     * @var int 
     */
    private $_idOrga = null;
    ...
    public function preDispatch(Zend_Controller_Request_Abstract $request) {
        ...
        //API : pas de gestion ACL pour l'API
        if ($reqModule == 'api') {
           return;
        }
        ...
        if ($reqModule=="admin") {
            $inscription = $reqController=='utilisateur' && $reqAction=='inscrire';
            $authentification = $reqAction=='authentifier' && $reqController=='utilisateur';
            $connecte = $this->_auth->hasIdentity();
            if (!$connecte && !$inscription && !$authentification) {
                //utilisateur non connecté, non inscrit, non authentifié
                $outModule      = "admin";
